...
배경: fable-mode 스택은 3중 활성화 경로를 가진다 — ① SessionStart 자동감지(모델이 opus면 세션 마커), ② `FABLE_MODE=1` 환경변수(claude-fablelike 래퍼 또는 직접 export), ③ 전역 마커(`~/.claude/state/fable-mode/GLOBAL`, 이 스킬이 관리). 활성 시 매 턴 규범 주입(fable-context.sh — 적응형: major 턴·세션 첫 주입·5회마다=풀 블록(턴 종료 전 '마무리 점검' 자가점검 불릿 포함), 그 외 minor 턴=압축 리마인더) + 서브에이전트 스폰 시 conduct 블록 주입(fable-subagent.sh, 빌트인·커스텀·워크플로 전부)이 걸린다. 풀 블록의 정체 단정("너는 Opus다")은 모델이 확정된 경로(env·트랜스크립트 감지)에서만 들어가고, 마커 추정뿐인 턴 1에는 중립 문구가 들어간다(v1.5). 트랜스크립트 감지는 마지막 어시스턴트 model 라인과 `/model` 커맨드 출력("Set model to …") 중 나중 신호를 우선한다 — 어시스턴트 라인만 보면 미드세션 /model 전환에 정확히 1턴 늦는다(v1.6). 턴 종료를 강제로 막던 Stop 훅(fable-stop-verify.sh)은 v1.5에서 은퇴한 no-op 스텁이다 — block reason이 사용자 화면에 에러처럼 노출되고 major 턴 생성이 2배가 되는 문제로, 자가점검은 풀 블록의 '마무리 점검' 불릿으로 이관됐다(스텁은 재활성화 대비로 Stop에 계속 등록). 주입마다 `~/.claude/state/fable-mode/stats/<세션id>.tsv`에 `타입<TAB>문자수` 한 줄이 기록된다.
