...
| 强制回合结束前自我验证 | ❌ | ❌ | ✅ Stop `decision:block` |
| 确定性泄漏防护 (用 grep 而非信任) | ❌ | ❌ | ✅ v1.4 |
| 自动检测 Opus，遇 Fable 会话自动待命 | ❌ | ❌ | ✅ |
| 注入开销遥测 | ❌ | ❌ | ✅ v1.4 |

## ⚙️ 工作原理

四个 shell 脚本抓住会话生命周期的四个时刻。没有守护进程，没有代理 —
状态就是空的标记文件。

```
  ┌──────────────────────────────────────────────────────────────┐
  │  SessionStart      fable-detect.sh                           │
  │    「现在是 Opus 吗?」— 标志 → settings → transcript 回退      │
  │           ↓                                                  │
  │  UserPromptSubmit  fable-context.sh          (每轮)         │
  │    重新注入 13 条行为规范 — v1.4 起自适应:                     │
  │    major 回合 = 完整块 · minor 回合 = −84% 提醒块             │
  │           ↓                                                  │
  │  SubagentStart     fable-subagent.sh         (每次生成)      │
  │    子代理看不到每轮注入 → 在生成时刻注入                        │
  │           ↓                                                  │
  │  Stop              fable-stop-verify.sh      (major 回合)    │
  │    拦截回合结束一次: 验证主张·结论优先重写 →                    │
  │    再 grep 最终文本，连规范泄漏也抓住                          │
  └──────────────────────────────────────────────────────────────┘
```
...
| 🏷️ 身份 | 注入块固定为「你是 Opus」— 任务输出中不冒充 Fable |

> [!IMPORTANT]
> Stop 自我验证会为**每个 major 提示多花一个回合**。这是工具包成本的大头，
> 也是在有超时预算的无头运行中应 `export FABLE_MODE=0` 的原因。
